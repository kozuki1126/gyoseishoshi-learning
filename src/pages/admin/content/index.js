  ...
  return (
    <tr className="hover:bg-gray-50">
      ...
      <td className="px-6 py-4">
        <span
          className={`rounded-full px-2 py-1 text-xs font-medium ${
            CONTENT_TYPE_BADGE_CLASSES[content.type] || 'bg-gray-100 text-gray-700'
          }`}
        >
      ...
      <td className="px-6 py-4">
        <span
          className={`rounded-full px-2 py-1 text-xs font-medium ${
            DIFFICULTY_BADGE_CLASSES[content.difficulty] || 'bg-gray-100 text-gray-700'
          }`}
        >
      ...
      <td className="px-6 py-4">
        <span className="text-sm text-gray-500">{formatUpdatedAt(content.updatedAt)}</span>
      </td>
      <td className="px-6 py-4">
...
export default function ContentList() {
  ...
  useEffect(() => {
    async function run() {
      ...
      try {
        ...
        if (filters.subject) {
          params.set('subjectId', filters.subject);
        }
        if (filters.type) {
        ...
        const res = await fetch(`/api/content/units?${params.toString()}`);
        ...
        const normalizedContents = data.units.map((unit) => ({
          ...unit,
          hasAudio: Boolean(unit.hasAudio || unit.audioUrl),
          hasPdf: Boolean(unit.hasPdf || unit.pdfUrl),
        }));

        const query = appliedSearchQuery.trim().toLowerCase();
        const matchedContents = query
          ? normalizedContents.filter((unit) => {
              const title = unit.title?.toLowerCase() || '';
              const subjectName = getSubjectName(unit.subjectId)?.toLowerCase() || '';
              return title.includes(query) || subjectName.includes(query) || unit.id.includes(query);
            })
          : normalizedContents;

        const total = matchedContents.length;
        const totalPages = Math.max(1, Math.ceil(total / PAGE_SIZE));
        const currentPage = Math.min(pagination.page, totalPages);
        const startIndex = (currentPage - 1) * PAGE_SIZE;

        setContents(matchedContents.slice(startIndex, startIndex + PAGE_SIZE));
        setPagination({
          page: currentPage,
          total,
          totalPages,
        ...
        setNotice({
          type: 'error',
          text: error.message || 'コンテンツの読み込みに失敗しました',
        });
      } finally {
        setLoading(false);
      }
    ...
    run();
  }, [filters, pagination.page, appliedSearchQuery]);
  ...
  function handleDelete(id) {
    if (!confirm('削除 API はまだ未接続です。削除操作の導線だけ確認しますか？')) {
      return;
    }

    setNotice({
      type: 'info',
      text: `単元 ${id} の削除 API はまだ未接続です。静的データとの整合確認後に有効化してください。`,
    });
  }
          ...
          <>
            ...
            <div className="flex items-center justify-between border-t border-gray-100 px-6 py-4">
              <p className="text-sm text-gray-500">
                {pagination.total} 件中 {(pagination.page - 1) * PAGE_SIZE + 1} -{' '}
                {Math.min(pagination.page * PAGE_SIZE, pagination.total)} 件を表示
              </p>
              <div className="flex items-center gap-2">
                <button
                  onClick={() =>
